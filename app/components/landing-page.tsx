...
export const THEMES: Record<string, ThemeConfig> = {
  ...
  forest: { name:"\u68ee\u6797\u7eff", heroFrom:"from-green-900", heroVia:"via-emerald-700", heroTo:"to-teal-800",
    ...
    accentBg:"bg-emerald-600", accentText:"text-white", accentHover:"hover:bg-emerald-500" }, name:"暖阳橙", heroFrom:"from-amber-800", heroVia:"via-orange-700", heroTo:"to-rose-800",
...
interface Props {
  data: LandingPageData; themeKey: string; showAdminBar?: boolean;
  onRegenerate?: () => void; shareUrl?: string; pageId?: string;
}
...
function buildHtml(data: LandingPageData): string {
  ...
  return '<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>'+esc(data.heroTitle)+'</title><script src="https://cdn.tailwindcss.com"></script></head><body class="min-h-screen bg-white text-gray-900">'+
    ...
    (data.services.length ? '<section class="py-20 px-6 border-b border-gray-100"><div class="max-w-6xl mx-auto text-center"><h2 class="text-3xl font-bold mb-4">服务项目</h2><p class="text-gray-500 mb-14">专业服务</p><div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">'+svc+'</div></div></section>' : '')+
    (data.advantages.length ? '<section class="py-20 px-6 bg-gray-50 border-b border-gray-100"><div class="max-w-6xl mx-auto text-center"><h2 class="text-3xl font-bold mb-4">为什么选择我们</h2><p class="text-gray-500 mb-14">我们与众不同</p><div class="grid gap-5 sm:grid-cols-2">'+adv+'</div></div></section>' : '')+
    '<section class="bg-gray-900 text-white py-20 px-6 text-center"><h2 class="text-4xl font-bold mb-4">准备好开始了吗？</h2><p class="text-lg text-gray-300 mb-8">'+esc(data.heroSubtitle)+'</p><a href="#" class="inline-block px-8 py-3 rounded-lg bg-white text-gray-900 font-semibold">'+esc(data.cta)+'</a></section>'+
...
export default function LandingPage({ data, themeKey, showAdminBar, onRegenerate, shareUrl, pageId }: Props) {
  ...
  const floatFields = data.formFields?.length ? data.formFields : [{label:"姓名",type:"text",required:true},{label:"电话",type:"tel",required:true},{label:"留言",type:"textarea",required:false}];
  ...
  const submitForm = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim() || !form.phone.trim()) { setFormErr("请填写姓名和电话"); return; }
    ...
    } catch { setFormErr("提交失败，请重试"); setFormStatus("error"); }
  };
  ...
  const generateQR = async () => {
    ...
    if (w) { w.document.write('<img src="'+url+'" style="width:300px;height:300px;display:block;margin:20px auto" /><p style="text-align:center;font-family:sans-serif;color:#666">扫一扫查看商家页面</p>'); w.document.title = "QR Code"; }
  };
  const downloadHtml = () => {
    ...
    <div className="min-h-screen bg-white" style={pageBgStyle}>
        ...
        <div className="sticky top-0 z-50 border-b border-gray-100 bg-white/80 backdrop-blur-sm">
          <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
            <Link href="/" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 12H5"/><polyline points="12 19 5 12 12 5"/></svg>返回
            </Link>
            <div className="flex items-center gap-2">
              <button onClick={()=>setShowInq(!showInq)}
                className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50">&#x1F4CB; 咨询 ({getInquiries().length})</button>
              {publicUrl && <button onClick={()=>navigator.clipboard.writeText(publicUrl)}
                className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-600 hover:bg-blue-100">&#x1F517; 公开链接</button>}
              {pageId && <button onClick={generateQR}
                className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50">&#x1F4F7; 二维码</button>}
              {shareUrl && <button onClick={copyLink} className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50">复制链接</button>}
              <button onClick={downloadHtml} className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50">导出</button>
              {onRegenerate && <button onClick={onRegenerate} className="rounded-lg bg-gray-900 px-4 py-1.5 text-sm font-medium text-white hover:bg-gray-800">重新生成</button>}
            </div>
          </div>
          {showInq && (
            <div className="border-t border-gray-100 bg-gray-50 px-6 py-4">
              <p className="text-sm font-semibold text-gray-900 mb-2">客户咨询</p>
              {getInquiries().length===0 ? <p className="text-xs text-gray-400">暂无咨询</p> :
      ...
      {services.length>0 && (
        <section className="border-b border-gray-100" style={data.sectionBgColors?.services ? {backgroundColor:data.sectionBgColors.services} : {}}>
          <div className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
            <h2 className="text-2xl font-bold text-center text-gray-900 sm:text-3xl mb-4">{st.services || "服务项目"}</h2>
            <p className="text-center text-gray-500 mb-14">专业服务，值得信赖</p>
      ...
      {advantages.length>0 && (
        <section className={"border-b border-gray-100 "+(data.sectionBgColors?.advantages ? "" : t.sectionBg)} style={data.sectionBgColors?.advantages ? {backgroundColor:data.sectionBgColors.advantages} : {}}>
          <div className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
            <h2 className="text-2xl font-bold text-center text-gray-900 sm:text-3xl mb-4">{st.advantages || "为什么选择我们"}</h2>
            <p className="text-center text-gray-500 mb-14">我们与众不同</p>
      ...
      <section className={"text-white "+(data.sectionBgColors?.cta ? "" : t.ctaBg)} style={data.sectionBgColors?.cta ? {backgroundColor:data.sectionBgColors.cta} : {}}>
        <div className="mx-auto max-w-4xl px-6 py-20 sm:py-28 text-center">
          <h2 className="text-3xl font-bold sm:text-4xl mb-4">{st.cta || "准备好开始了吗？"}</h2>
          <p className="text-lg text-gray-300 max-w-xl mx-auto mb-8">{heroSubtitle}</p>
          <button onClick={scrollToContact} className={"rounded-lg px-8 py-3 text-sm font-semibold shadow-sm transition-colors "+t.accentBg+" "+t.accentText+" "+t.accentHover}>立即咨询</button>
        </div>
      </section>
      ...
      <section id="contact-form" className="border-b border-gray-100" style={data.sectionBgColors?.contact ? {backgroundColor:data.sectionBgColors.contact} : {}}>
        <div className="mx-auto max-w-3xl px-6 py-20 sm:py-28">
          <h2 className="text-2xl font-bold text-center text-gray-900 sm:text-3xl mb-2">{st.contact || "联系我们"}</h2>
          <p className="text-center text-gray-500 mb-10">填写信息，我们将尽快与您联系</p>
          {formStatus==="success" ? (
            <div className="text-center py-12"><div className="text-5xl mb-4">&#x2705;</div><p className="text-lg font-semibold text-gray-900 mb-2">提交成功！</p><p className="text-sm text-gray-500 mb-6">我们会尽快与您取得联系。</p><button onClick={()=>setFormStatus("idle")} className="text-sm text-gray-500 underline">继续咨询</button></div>
          ) : (
            <form onSubmit={submitForm} className="space-y-5 max-w-lg mx-auto">
              <input type="text" placeholder="您的姓名" value={form.name} required onChange={e=>setForm(p=>({...p,name:e.target.value}))}
              ...
              <input type="tel" placeholder="您的手机号" value={form.phone} required onChange={e=>setForm(p=>({...p,phone:e.target.value}))}
                className="block w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-gray-900 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" />
              <textarea rows={3} placeholder="请描述您的需求" value={form.message} onChange={e=>setForm(p=>({...p,message:e.target.value}))}
              ...
              <button type="submit" disabled={formStatus==="submitting"}
                className="w-full rounded-lg bg-gray-900 px-6 py-3 text-sm font-semibold text-white shadow-sm hover:bg-gray-800 disabled:opacity-50">{formStatus==="submitting" ? "提交中..." : "提交咨询"}</button>
            </form>
          )}
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={()=>setShowFloatForm(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full max-h-[85vh] overflow-y-auto shadow-xl" onClick={e=>e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900">立即咨询</h3>
            ...
            <form onSubmit={(e)=>{e.preventDefault(); const fd:Record<string,string>={}; const els=e.currentTarget.elements; for(let i=0;i<els.length;i++){const el=els[i] as HTMLInputElement; if(el.name) fd[el.name]=el.value;} fetch("/api/contact",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...fd,business:heroTitle})}).then(()=>setShowFloatForm(false)).catch(()=>{})}} className="space-y-4">
              {floatFields.map((f, i) => (
                <div key={i}>
                  ...
                  {f.type === "textarea" ? (
                    <textarea rows={3} name={f.label} required={f.required} placeholder="请输入" className="block w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-gray-900 text-sm resize-none" />
                  ) : (
                    <input type={f.type} name={f.label} required={f.required} placeholder="请输入" className="block w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-gray-900 text-sm" />
                  )}
                </div>
              ))}
              <button type="submit" className="w-full rounded-lg bg-gray-900 px-6 py-3 text-sm font-semibold text-white hover:bg-gray-800">提交咨询</button>
            </form>
          </div>
        </div>
      )}
